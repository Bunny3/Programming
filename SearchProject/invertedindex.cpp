...
map<string,int> mp;
map<map<string,int>,struct node *> nmp;
...
void fun(fstream& fio,int docID)
{
  ...
  map<map<string,int>,struct node *>::iterator ftr;
  for(ftr=mp.begin();ftr!=mp.end();ftr++)
  {
    map<string,int> data;
    string str=ftr->first;
    int freq=ftr->second;
    data.insert(str,freq);
    struct node *temp=newnode(docID);
    if(nmp.count(data)==0)
    {
      nmp.emplace(data,temp);
    }
    // else
    // {
    //   insertNode(nmp[data],temp);
    // }
  }
}
int main()
